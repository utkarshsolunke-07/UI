...
export class WebGLVideoEngine {
  ...
  _initPrograms() {
    ...
    const fsBlit    = this._compileShader(gl.FRAGMENT_SHADER, this._fsBlit());
    ...
    this.progBlit    = this._linkProgram(vsFBO,   fsBlit);
    ...
    this.locEASU = {
      ...
      swapRB:  gl.getUniformLocation(this.progEASU, 'u_swapRB'),
    };

    this.locBlit = {
      src:     gl.getUniformLocation(this.progBlit, 'u_src'),
    };
  ...
  _initVAO() {
    ...
    if (this.isWebGL2) {
      for (const [name, prog] of [
        ...
        ['taa', this.progTAA], ['blit', this.progBlit]
      ]) {
  ...
  render(videoSource, settings = {}) {
    ...
    if (this.locEASU.swapRB)  gl.uniform1i(this.locEASU.swapRB, settings.swapRB ? 1 : 0);
    ...
    if (enableTAA) {
      ...
      if (this.isWebGL2) gl.bindVertexArray(this.vaos.blit);
      else this._bindAttributes(this.progBlit);
      
      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, writeHistTex);
      gl.uniform1i(this.locBlit.src, 0);
      gl.drawArrays(gl.TRIANGLES, 0, 6);
    } else {
