...
export default function VideoStudio({ settings, setSettings }) {
  ...
  const stageRef        = useRef(null);    // Viewport card container reference
  ...
  return (
    ...
    >
          ...
          <div className="panel-body">
            ...
            <button
              ...
              style={{
                ...
                fontSize: '0.7rem',
                padding: '0.55rem',
                border: settings.swapRB ? '1px solid #38bdf8' : '1px solid rgba(255,255,255,0.1)',
                background: settings.swapRB ? 'rgba(56,189,248,0.2)' : 'rgba(0,0,0,0.2)',
                color: settings.swapRB ? '#38bdf8' : 'var(--text)',
                fontWeight: 800
              }}
              onClick={() => set('swapRB', !settings.swapRB)}
            >
              🔁 FIX BLUE TINT (R-B SWAP): {settings.swapRB ? 'ON (CORRECTED)' : 'OFF (DEFAULT)'}
            </button>
      ...
      {/* ============================================================
         ...
         ============================================================ */}
      <div ref={stageRef} className="viewport-card" style={{ marginTop: '0.5rem' }}>
        {/* Toolbar */}
        <div className="viewport-toolbar">
          ...
          <div className="viewport-toolbar-right">
            <span style={{ display: 'flex', gap: '0.4rem', marginRight: '0.5rem' }}>
              ...
              <button
                className={`btn-secondary ${settings.swapRB ? 'active' : ''}`}
                style={{ fontSize: '0.68rem', padding: '0.35rem 0.75rem', background: settings.swapRB ? 'rgba(56,189,248,0.2)' : '', border: settings.swapRB ? '1px solid #38bdf8' : '', color: settings.swapRB ? '#38bdf8' : '' }}
                onClick={() => set('swapRB', !settings.swapRB)}
                title="Swaps Red & Blue channels to fix ANGLE GPU video tint"
              >
                🔁 FIX BLUE TINT
              </button>
              <button
                ...
                title="Expand Viewport Stage to Fullscreen Cinema Mode"
              >
                📺 FULLSCREEN STAGE
              </button>
            </span>
